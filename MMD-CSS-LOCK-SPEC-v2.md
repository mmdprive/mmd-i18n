...
### 1.2 Safe Layer Contract
กติกาชุดนี้เป็น **HARD LOCK** เพราะเป็น boundary ที่ป้องกันไม่ให้ CSS ลามทั้งเว็บ

#### Wrapper Contract
ทุกหน้าที่ต้องการใช้ระบบ MMD ต้องครอบด้วย:
...
ลำดับการเรียก CSS ต้องคงที่ดังนี้:
...
- `mmd-membership.css` และ `mmd-payment.css` เป็น layer ถัดไป
- page-specific CSS ต้องเรียกหลังสามไฟล์นี้เสมอ

#### Scoped-only Rule
component/global style ของระบบ MMD ต้องอยู่ใต้ `.mmd-system` เท่านั้น
...
#### Global Override Ban
ห้ามมี selector กว้างใน global สำหรับระบบ MMD เช่น
...
เว้นแต่มี `.mmd-system` scope นำหน้าอย่างชัดเจน

### 1.3 Token Naming Contract
...
### 1.4 Component Class Naming Contract
...
### 1.8 Primary / Secondary Action Hierarchy
ลำดับปุ่มต้องคงที่
...
### 2.6 Membership / Payment Layer Tuning
ปรับได้:
- package card emphasis
- method card layout
- confirm page spacing
- instruction block layout
- upload box arrangement

โดยห้ามชนกับ Hard Lock layer
...
- ไม่ลามไปกระทบหน้าที่ไม่ได้ใช้ `.mmd-system`
...
- และต้องไม่ทำลายหน้าอื่นของเว็บ
